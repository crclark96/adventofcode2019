...
numThirdElem :: (Eq a) => a -> [a] -> Int
numThirdElem x xs = length $ Prelude.filter (\(i,z) -> (i`mod`3 == 2 && z==x)) $ zip [0..] xs

main = do
  contents <- readFile "input/input13"
  let input = M.fromList $ zip [0..] $ Prelude.map (read :: String -> Int) $ words [if x == ',' then ' ' else x | x <- contents]
      ...
      state = State { mem = input, output = [], input = [], base = 0 }
      o     = reverse $ output $ runop 0 state
  print $ numThirdElem 2 o
